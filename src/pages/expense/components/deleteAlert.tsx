...
export function ExpenseDeleteAlert() {
  const queryClient = useQueryClient();
  const deleteAlertData = useExpenseStore(
    (state) => state.deleteExpenseAlertData,
  );
  const setDeleteAlertOpen = useExpenseStore(
    (state) => state.setDeleteExpenseAlertData,
  );
  const mutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await del(`/expenses/${id}`);
    ...
    >
      <AlertDialogContent size="sm">
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>
            Deletar {deleteAlertData.expense?.name}?
          </AlertDialogTitle>
          <AlertDialogDescription>
            Tem certeza que deseja deletar essa despesa? Essa ação não pode ser
            desfeita.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            onClick={() => mutation.mutate(deleteAlertData.expense?.id || 0)}
            asChild
          >
